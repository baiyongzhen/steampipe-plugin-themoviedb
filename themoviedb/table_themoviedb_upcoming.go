...
* https://github.com/cyruzin/golang-tmdb/blob/master/results.go
* MovieNowPlayingResults Result Types
 */
func tableMovieUpcoming() *plugin.Table {
	return &plugin.Table{
		Name: "themoviedb_upcoming",
		Description: "Get a list of movies that are being released soon",
		List: &plugin.ListConfig{
			Hydrate: listMovieUpcoming,
		},
		Get: &plugin.GetConfig{
			KeyColumns: plugin.SingleColumn("id"),
			Hydrate:    getMovieUpcoming,
		},
...
func listMovieUpcoming(ctx context.Context,
	...
	logger.Info("Log message and a variable", language)
	logger.Info("Log message and a variable", language)
	logger.Info("Log message and a variable", language)
	logger.Info("Log message and a variable", language)
	...
	for {
		movies, err := conn.GetMovieUpcoming(options)
		if err != nil {
			logger.Trace("GetMovieUpcoming Error:", err.Error())
			return nil, err
		}
...
func getMovieUpcoming(ctx context.Context,
	d *plugin.QueryData,
	...
	quals := d.EqualsQuals
	//steampipe logger
	logger.Warn("getMovieUpcoming", "quals", quals)
	id := quals["id"].GetInt64Value()
	logger.Warn("getMovieUpcoming", "id", id)
	return nil, nil
}
